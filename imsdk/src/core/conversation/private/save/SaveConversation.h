...
public:
    ...
    static boost::asio::awaitable<std::vector<std::shared_ptr<model::ConversationModel>>> 
        save_net_conversations(W_SDK_ROOT, std::vector<std::shared_ptr<network::ConversationInfo>> convs);
    
    /// 根据 ID 获取 SDK 会话
    static boost::asio::awaitable<std::shared_ptr<model::ConversationModel>> 
        sdk_conv_for_id(W_SDK_ROOT, const std::string &conv_id);
...
private:
    /// 更新会话缓存 (非线程安全， )
    static boost::asio::awaitable<std::vector<std::shared_ptr<model::ConversationModel>>> 
        update_conv_cache(W_SDK_ROOT, std::vector<std::shared_ptr<roc::imsdk::model::ConversationModel>> sdk_convs);
};
