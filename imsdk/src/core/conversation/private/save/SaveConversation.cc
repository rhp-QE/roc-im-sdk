...
SaveConversation::SaveNetConversations(CONTEXT_T, std::vector<std::shared_ptr<network::ConversationInfo>> convs) {
    ...
    auto db_convs = base::util::transform(convs, [](const std::shared_ptr<network::ConversationInfo> &conv) {
        return core::conversation::Convert::ConvertNetConvToDbConv(conv.get());
    });

    // 转换为 sdk 会话
    auto sdk_convs_copy = base::util::transform(db_convs, [=](const std::shared_ptr<core::conversation::ConversationORM> &conv) {
        return core::conversation::Convert::ConvertDbConvToSdkConv(CONTEXT_V, conv.get());
    });

    auto conv_manager = sdk_root->ConversationManager();

    auto sdk_convs = co_await boost::asio::co_spawn(conv_manager->ConvStrand(), [=, sdk_convs_copy = std::move(sdk_convs_copy)]() -> boost::asio::awaitable<std::vector<std::shared_ptr<model::ConversationModel>>> {
        CHECK_ROOT_OR_CO_RETURN_VALUE(w_sdk_root, std::vector<std::shared_ptr<model::ConversationModel>>());

        /// 保存到数据库
        conversation::DBOpt::InsertConversation(CONTEXT_V, db_convs);

        /// 更新会话缓存
        co_return co_await p_UpdateConvCache(CONTEXT_V, std::move(sdk_convs_copy));
...
boost::asio::awaitable<std::shared_ptr<model::LoadUserConvsResult>> 
    ...
    auto sdk_convs = co_await boost::asio::co_spawn(conv_manager->ConvStrand(), [=]() -> boost::asio::awaitable<std::vector<std::shared_ptr<model::ConversationModel>>> {
        CHECK_ROOT_OR_CO_RETURN_VALUE(w_sdk_root, std::vector<std::shared_ptr<model::ConversationModel>>());

        /// 从DB 中获取会话
        auto sdk_convs_copy = conversation::DBOpt::QueryConversations(CONTEXT_V, cursor, limit, forward);

        /// 更新会话缓存
        auto sdk_convs = co_await p_UpdateConvCache(CONTEXT_V, std::move(sdk_convs_copy));
...
boost::asio::awaitable<std::shared_ptr<model::ConversationModel>> SaveConversation::SdkConvForId(CONTEXT_T, const std::string &conv_id) {
    ...
    auto sdk_conv = co_await boost::asio::co_spawn(conv_manager->ConvStrand(), [=]() -> boost::asio::awaitable<std::shared_ptr<model::ConversationModel>> {
        CHECK_ROOT_OR_CO_RETURN_VALUE(w_sdk_root, nullptr);
        ...
        /// 从DB 中获取会话
        auto sdk_conv_copy = conversation::DBOpt::ConversationForId(CONTEXT_V, conv_id);

        /// 更新缓存
        auto sdk_convs = co_await p_UpdateConvCache(CONTEXT_V, {std::move(sdk_conv_copy)});
