...
public:
    /// 发送消息
    static boost::asio::awaitable<std::shared_ptr<model::SendMessageResponse>> send_message(W_SDK_ROOT, model::SendMsgContext context, std::function<void(std::shared_ptr<model::SendMessageResponse>)> callback);
 
private: 
    /// 异步发送消息
    static boost::asio::awaitable<void> async_send_message(W_SDK_ROOT, std::unique_ptr<network::SendMessageReq> req, std::function<void(std::shared_ptr<model::SendMessageResponse>)> callback);
